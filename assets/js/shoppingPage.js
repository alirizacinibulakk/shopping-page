...
function render(){
  ...
  for (const product of products){
    if(product.title.toLocaleLowerCase("en").includes(searchFilter) !== true){
      continue;
    }
    productList.innerHTML += 
    `
    <li class="product-card">
      ...
      <div class="product-footer">
        <strong>$${product.price}</strong> | ${product.stock}
      </div>
      <ul class="product-tags">
        ${getProductTagsHtml(product.tags)}
      </ul>
    </li>
    ` 
  }
...
function handleSubmit(e){
  e.preventDefault();
  console.log(searchBar.filteredSearch.value);
  searchFilter = searchBar.filteredSearch.value;
  render();
}
